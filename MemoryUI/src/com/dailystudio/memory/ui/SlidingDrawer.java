...
public class SlidingDrawer extends ViewGroup {
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (mTracking || mAnimating || mBottomOffset < 0) {
    ...
    private void prepareContent() {
		...
		if (Build.VERSION.SDK_INT >= 11) { // Android v3.0+
			try {
				Method method = View.class.getMethod("isHardwareAccelerated");
				Object o = method.invoke(this);
				
				if (o instanceof Boolean) {
					hardwareAccelerated = ((Boolean)o).booleanValue();
				}
			} catch (Exception e) {
				Logger.warnning("isHardwareAccelerated() failure: %s", e.toString());
			}
		}

        if (!hardwareAccelerated) content.buildDrawingCache();
